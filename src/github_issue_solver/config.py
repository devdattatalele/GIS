...
class Config:
    ...
    def __init__(self, env_file: Optional[str] = None):
        ...
        # LLM provider detection (needed before validation)
        self._llm_provider = os.getenv("LLM_PROVIDER", "gemini").lower()
        self._embedding_provider = os.getenv("EMBEDDING_PROVIDER", "google").lower()

        # Required environment variables (conditionally built)
        self._required_vars = {}

        # API key requirements based on provider
        _provider_api_keys = {
            "gemini":  ("GOOGLE_API_KEY",    "Google API key for Gemini LLM"),
            "claude":  ("ANTHROPIC_API_KEY", "Anthropic API key for Claude LLM"),
            "grok":    ("XAI_API_KEY",       "xAI API key for Grok LLM"),
            "openai":  ("OPENAI_API_KEY",    "OpenAI API key"),
            # ollama = local, no key needed
        }
        if self._llm_provider in _provider_api_keys:
            key, desc = _provider_api_keys[self._llm_provider]
            self._required_vars[key] = desc
        # Google key also required if using google embeddings
        if self._embedding_provider == "google" and "GOOGLE_API_KEY" not in self._required_vars:
            self._required_vars["GOOGLE_API_KEY"] = "Google API key for embeddings"
        self._required_vars["GITHUB_TOKEN"] = "GitHub personal access token for repository access"
        ...
        self._optional_vars = {
            ...
            "LLM_MODEL_NAME": "",  # Override model (default per provider via LiteLLM)
        }
    ...
    def _load_values(self) -> None:
        ...
        # API keys (may be None if not needed for chosen provider)
        ...
        self.llm_model_name = os.getenv("LLM_MODEL_NAME", "") or None  # None means use provider default
    ...
    def _setup_license_and_user(self) -> None:
        ...
        try:
            ...
            # Get persistent machine ID for trial users (for tracking and limit enforcement)
            if self.license_info.is_trial:
                from .license import get_persistent_machine_id
                self.machine_id = get_persistent_machine_id()
                logger.info(f"Trial user - Persistent Machine ID: {self.machine_id[:8]}...")
    ...
    def get_collection_name(self, repo_name: str, collection_type: str) -> str:
        ...
        """
        # Guard against None repo_name
        if not repo_name:
            raise ValueError("repo_name cannot be None or empty")
    ...
    def validate_api_access(self) -> Dict[str, bool]:
        ...
        # Test Google API (only if configured)
        if self.google_api_key:
            ...
            results["google_api"] = None  # Not configured

        # LLM provider info
        results["llm_provider"] = self.llm_provider
