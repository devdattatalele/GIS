...
class PatchService:
    ...
    def __init__(self, config: Config, state_manager: StateManager, llm_service=None):
        """
    ...
    async def generate_code_patch(self, issue_body: str, repo_full_name: str) -> PatchResult:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Generating patches using repository knowledge base for {repo_full_name} (LLM: {self.llm_service.provider_name if self.llm_service else 'default gemini'})")
                patch_data = await asyncio.to_thread(generate_patch_for_issue, issue_body, repo_full_name, llm)
