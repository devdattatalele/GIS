...
class AnalysisService:
    ...
    def __init__(self, config: Config, repository_service: RepositoryService, state_manager: StateManager, llm_service=None):
        """
        ...
        Args:
            ...
            llm_service: Optional LLMService for provider-agnostic LLM access
        """
        self.config = config
    ...
    async def analyze_github_issue(self, issue_url: str) -> AnalysisResult:
        ...
        try:
            ...
            try:
                llm = self.llm_service.get_llm() if self.llm_service else None
                logger.info(f"Creating LangChain agent for analysis (LLM: {self.llm_service.provider_name if self.llm_service else 'default gemini'})...")
                agent_raw_output = await asyncio.to_thread(create_langchain_agent, mock_issue, llm)
    ...
    def _create_detailed_report(self, issue_info: IssueInfo, analysis: Dict[str, Any], timestamp: str) -> str:
        """
        Create a detailed analysis report with precise file locations.
        ...
        if affected_files:
            ...
            for f in affected_files:
                if isinstance(f, dict):
                    files_section += f"- `{f.get('file_path', 'unknown')}` (lines {f.get('line_numbers', '?')}): {f.get('description', '')}\n"
                else:
                    files_section += f"- {f}\n"
        else:
